...
//
//info.plist 权限添加：定位权限(扫描iBeacon需精确定位)，相机权限(实景导航)，语音权限(可选)，网络权限(可选)。
/*
...
+ (UIViewController *)createWebViewController:(UIViewController *)vc url:(id)url delegate:(id<WKNavigationDelegate,WKUIDelegate>)delegate onJSCall:(void(^)(NSString *,id,void(^callback)(NSString *)))onJSCall;
